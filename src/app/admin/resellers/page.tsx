...
export default function AdminResellersPage() {
  ...
  const load = useCallback(async () => {
    ...
    let q = supabase
      .from('profiles')
      .select('id, status, full_name, company, country, created_at, resellers(commission_rate, referral_code, total_orders, total_commission, customer_count, notes)')
      .eq('role', 'reseller')
    if (filter !== 'all') q = q.eq('status', filter)
    const { data } = await q.order('created_at', { ascending: false })
    const mapped = (data ?? []).map((r: Record<string, unknown>) => {
      const s = r.resellers as Record<string, unknown> | null
      return {
        id: r.id as string,
        ...
        commission_rate: (s?.commission_rate as number) ?? 10,
        referral_code: (s?.referral_code as string) ?? '',
        total_orders: (s?.total_orders as number) ?? 0,
        total_commission: (s?.total_commission as number) ?? 0,
        customer_count: (s?.customer_count as number) ?? 0,
  ...
  function openDetail(row: ResellerRow) {
    setSelected(row)
  ...
  const displayed = rows.filter((r) => {
    ...
    return (r.full_name ?? '').toLowerCase().includes(s) || (r.company ?? '').toLowerCase().includes(s)
  })
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Manage reseller/agent partners</p>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="table-th">Customers</th>
              <th className="table-th">Orders</th>
              <th className="table-th">Commission Earned</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {displayed.length === 0 && (
              <tr><td colSpan={9} className="px-4 py-10 text-center text-gray-400">No resellers found</td></tr>
            )}
            {displayed.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50">
                ...
                <td className="table-td">{row.customer_count}</td>
                <td className="table-td">{row.total_orders}</td>
                <td className="table-td">{formatMoney(row.total_commission)}</td>
                <td className="table-td">
                ...
                <td className="table-td">
                  <div className="flex gap-2">
                    <button onClick={() => openDetail(row)} className="text-xs btn-secondary py-1 px-2">Manage</button>
                    {row.status === 'pending' && (
                      <button onClick={() => updateStatus(row.id, 'active')} className="text-xs bg-green-50 hover:bg-green-100 text-green-700 border border-green-200 py-1 px-2 rounded-lg">Approve</button>
                    )}
                    {row.status === 'active' && (
                    ...
                    {row.status === 'suspended' && (
                      <button onClick={() => updateStatus(row.id, 'active')} className="text-xs bg-green-50 hover:bg-green-100 text-green-700 border border-green-200 py-1 px-2 rounded-lg">Reinstate</button>
                    )}
                  </div>
                </td>
              </tr>
      ...
      <Modal
        ...
        title={`Reseller: ${selected?.company ?? selected?.full_name}`}
        footer={
          <>
        ...
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-gray-500">Customers:</span> {selected.customer_count}</div>
              <div><span className="text-gray-500">Orders:</span> {selected.total_orders}</div>
              <div><span className="text-gray-500">Commission Earned:</span> {formatMoney(selected.total_commission)}</div>
              <div>
                <span className="text-gray-500">Referral Code:</span>{' '}
                <span className="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{selected.referral_code || '—'}</span>
              </div>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-400 mt-1">Reseller earns this % of each order from their customers</p>
            </div>
            ...
            {selected.status === 'pending' && (
              <div className="flex gap-2 pt-2">
                <button onClick={() => updateStatus(selected.id, 'active')} disabled={saving} className="btn-primary flex-1">✓ Approve</button>
                <button onClick={() => setModal('reject')} className="btn-danger flex-1">✗ Reject</button>
              </div>
            )}
          </div>
        )}
